...
class ExecutionAdapterTests {
  ...
  private val outbox = ExecutionAdapter(repository, wakeupService, meterRegistry, partitionObservers)
  ...
  fun `processNext increments rateLimitedCounter and pauses partition status gauge on rate limited`() {
    ...
    outbox.dispatchNext(partition) { OutboxTaskResult.RateLimited(java.time.Duration.ofSeconds(30)) }
  ...
  fun `processNext increments rateLimitedCounter but does not update gauge when pauseOnRateLimit is false`() {
    ...
    outbox.dispatchNext(noPausePartition) { OutboxTaskResult.RateLimited(java.time.Duration.ofSeconds(30)) }
